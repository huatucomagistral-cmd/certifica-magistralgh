...
import {
  ...
  Minus, Plus as PlusIcon, ChevronUp, ChevronDown, Link2,
} from "lucide-react";
...
function resolveDataKey(label: string, id: string): string {
  ...
  // Si el label tiene más de 4 palabras, es un campo de texto estático (no dato dinámico)
...
export default function NewTemplatePage() {
  ...
  // Sync refs with state (refs son accesibles en event listeners sin closures)
  ...
  useEffect(() => {
    const onMouseMove = (e: MouseEvent) => {
      ...
      // Detectar en qué página está el mouse y calcular ratios relativos a esa página
      for (let i = 0; i < pageRefs.current.length; i++) {
        const pageEl = pageRefs.current[i];
        if (!pageEl) continue;
        const rect = pageEl.getBoundingClientRect();
        // Usar página más cercana: verificar si el mouse está dentro (con margen)
        if (e.clientX >= rect.left && e.clientX <= rect.right) {
          // Para Y: clampar al rango de la página si está fuera verticalmente
          const pageNum = i + 1;
          ...
          // Si el mouse está dentro de la página verticalmente, o es la más cercana
  ...
  const [fields, setFields] = useState<EditableField[]>([
    { id: uuidv4(), label: "Nombre del Alumno", dataKey: "alumno_nombre", fontSize: 36, color: "#000000", fontFamily: "PlayfairDisplay", bold: true,  italic: false, align: "center" },
    { id: uuidv4(), label: "Nombre del Curso",  dataKey: "curso_nombre",  fontSize: 22, color: "#000000", fontFamily: "Helvetica",       bold: false, italic: true,  align: "center" },
    { id: uuidv4(), label: "Fecha de Emisión",  dataKey: "fecha_emision", fontSize: 16, color: "#000000", fontFamily: "Helvetica",       bold: false, italic: false, align: "left"   },
  ]);
  ...
  const [vuFontSize, setVuFontSize] = useState(7);
  const [vuColor,    setVuColor]    = useState("#555555");
  ...
  const handleImageClick = (e: React.MouseEvent<HTMLDivElement>, pageNum: number) => {
    ...
    if (activeId === "qr") {
      ...
      // Avanzar al siguiente campo sin posicionar
  ...
  const addField = () => {
    ...
    setFields((p) => [
      ...
      { id: newId, label, dataKey: resolveDataKey(label, newId), fontSize: 18, color: "#000000", fontFamily: "Helvetica", bold: false, italic: false, align: "left", page: activePage },
    ]);
    setActiveId(newId);
  };
  ...
  const updateField = (id: string, key: keyof EditableField, value: unknown) => {
    setFields((p) => p.map((f) => {
      ...
      // Si cambia el label, recalcular el dataKey automáticamente (solo si aún no es un custom personalizado manualmente)
      if (key === "label" && typeof value === "string") {
        const newKey = resolveDataKey(value, id);
        updated.dataKey = newKey;
      }
      return updated;
    }));
  };
                ...
                return (
                   <div
                    ...
                    >
                     ...
                     >
                      ...
                      {fields.filter((f) => f.xRatio !== undefined && (f.page ?? 1) === pageNum).map((field) => (
                        <div
                          key={field.id}
                          className={`absolute pointer-events-auto ${activeId === field.id ? "ring-2 ring-indigo-500 bg-indigo-500/5 shadow-2xl z-20" : "hover:ring-1 hover:ring-slate-400 z-10"} cursor-grab active:cursor-grabbing transition-shadow`}
                          onMouseDown={(e) => {
                              e.stopPropagation();
                      ...
                      {/* QR overlay */}
                      {qrXRatio !== undefined && qrYRatio !== undefined && qrPage === pageNum && (
                        <div
                    ...
                    <div className="absolute bottom-4 right-4 bg-white/90 backdrop-blur-md px-3 py-1.5 rounded-lg border border-slate-200 text-[10px] font-bold text-slate-500 shadow-sm pointer-events-none">
                      Cara {pageNum} · {fields.filter(f => (f.page ?? 1) === pageNum).length} campos · QR {qrXRatio !== undefined && qrPage === pageNum ? "✓" : "×"} · URL {vuXRatio !== undefined && vuPage === pageNum ? "✓" : "×"}
                    </div>
                  </div>
                );
              })}
